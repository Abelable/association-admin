...
export const BannerModal = () => {
  ...
  const editingApplicationForm = useEditingApplicationForm(editingBannerId);

  const useMutationApplication = editingBannerId
    ? useEditApplication
    : useAddApplication;
  ...
  return (
    <Drawer
      title={editingBannerId ? "编辑申请列表" : "新增申请列表"}
      size={"large"}
      forceRender={true}
      onClose={closeModal}
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item
              name="sort"
              label="头图排序"
              rules={[{ required: true, message: "请输入头图排序" }]}
            >
              <Input placeholder="请输入头图排序" />
            </Form.Item>
          </Col>
        </Row>
        <Row gutter={16}>
...
const useEditingApplicationForm = (editingBannerId: string) => {
  ...
  const currentApplication = applicationsResult
    ? applicationsResult.list.find((item) => item.id === editingBannerId)
    : undefined;
